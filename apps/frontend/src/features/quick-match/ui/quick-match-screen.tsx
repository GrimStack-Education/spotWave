...
export function QuickMatchScreen() {
  const eventsQuery = useQuery({ queryKey: queryKeys.events('quick-match'), queryFn: () => fetchEvents({ limit: 1 }) });
  ...
  if (!eventData) return <EmptyState title="Нет событий для quick match" description="Появятся после создания или расширения радиуса." />;
