...
export function OnboardingScreen() {
  ...
  const interestsQuery = useQuery({ queryKey: ['onboarding', 'interests'], queryFn: fetchOnboardingInterests });
      ...
      <UiCard className="relative overflow-hidden p-6 md:p-8">
        ...
        <div className="relative mt-8 space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <span className="rounded-full border border-white/10 bg-white/[0.05] px-3 py-1 text-xs text-white/56">Выбрано: {currentIds.length}</span>
          </div>
          ...
          {!availableInterests.length ? <p className="text-sm text-white/50">Интересы пока не заполнены в системе, можно продолжить без выбора.</p> : null}
        </div>
            ...
            {radii.map((radius) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="mt-2 block text-sm text-white/48">{radius === 2 ? 'Ближайшие места' : radius === 5 ? 'Оптимальный город' : 'Шире круг'}</span>
                </button>
              );
            })}
          </div>
        ...
        <UiButton className="relative mt-8 h-14 w-full md:w-auto" isDisabled={!canContinue} onClick={() => mutation.mutate({ radiusKm: currentRadius, interestIds: currentIds })}>
          <MapPin size={18} /> {mutation.isPending ? 'Сохраняем...' : 'Сохранить и продолжить'}
        </UiButton>
      </UiCard>

      <UiCard className="p-6">
        <CoverImage className="h-64 rounded-[26px]" seed="onboarding" priority alt="SpotWave onboarding" />
        ...
        <div className="mt-5 grid gap-3">
          <SideNote icon={<Radar size={16} />} title="Радиус влияет на карту" text="2/5/10 км явно меняют локальный контекст." />
          <SideNote icon={<Wand2 size={16} />} title="Интересы формируют фид" text="Выбранные темы дают алгоритму первый сигнал." />
        </div>
      </UiCard>
    </div>
  );
...
function SideNote({ icon, title, text }: { icon: ReactNode; title: string; text: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
      <p className="flex items-center gap-2 text-sm text-white/80">{icon}{title}</p>
