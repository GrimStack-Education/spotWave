  ...
  return (
    <div className="min-w-0 space-y-6">
      <div className="max-w-215">
        ...
        <p className="mt-5 max-w-155 text-lg leading-8 text-white/62 md:text-xl">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {stats.map((stat) => (
          <UiCard key={stat.label} className="p-5">
            ...
            <p className="mt-3 text-5xl leading-none tracking-tighter text-brand">{stat.value}</p>
          </UiCard>
        ))}
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_420px]">
        <UiCard className="p-5 md:p-6">
          <h2 className="text-3xl tracking-tighter">Ближайшие действия</h2>
          <div className="mt-5 grid gap-3">
            {[
              ...
              <p
                key={item}
                className="rounded-2xl border border-white/10 bg-white/3.5 p-4 text-white/64"
              >
                {item}
              </p>
            ))}
        ...
        <UiCard className="p-5 md:p-6">
          <h2 className="flex items-center gap-2 text-3xl tracking-tighter">
            <MessageCircle size={24} className="text-brand" /> Коммуникация
          </h2>
          <p className="mt-4 leading-7 text-white/58">
            Для демо этот раздел показывает, какие сигналы должен видеть организатор: заявки,
            сообщения, рейтинг и реальные посещения.
          </p>
        </UiCard>
      </div>
    </div>
