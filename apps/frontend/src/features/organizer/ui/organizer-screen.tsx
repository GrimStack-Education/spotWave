      ...
      <div className="grid gap-3 md:grid-cols-3">
        {stats.map((stat) => (
          <UiCard key={stat.label} className="p-5">
            ...
            <p className="mt-3 text-5xl leading-none tracking-[-0.05em] text-[var(--sw-accent-3)]">
              {stat.value}
            </p>
          </UiCard>
        ))}
        ...
        <UiCard className="p-5 md:p-6">
          ...
          <div className="mt-5 grid gap-3">
            {[
              ...
              <p
                key={item}
                className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 text-white/64"
              >
                {item}
              </p>
            ))}
