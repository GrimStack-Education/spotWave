...
export function CreateEventScreen() {
  ...
  const displayableTags = availableTags
    ...
    .toSorted((left, right) => left.name.length - right.name.length || left.name.localeCompare(right.name));
  ...
  const visibleTags = normalizedTagSearch
    ? displayableTags.filter((tag) => {
      ...
      return label.includes(normalizedTagSearch) || tag.name.toLowerCase().includes(normalizedTagSearch);
    })
    : displayableTags;
  ...
  return (
    <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
      <div className="relative min-w-0 overflow-hidden rounded-[26px] border border-white/10 bg-[var(--sw-neutral-800)] p-5 md:rounded-[34px] md:p-8">
        ...
        <h1 className="relative text-[44px] leading-[0.98] tracking-[-0.04em] text-white md:text-[74px] md:tracking-[-0.06em]">Создать событие</h1>
        <p className="relative mt-4 max-w-2xl text-white/58">Соберите встречу по интересам, отметьте место на карте и добавьте темы, чтобы участникам было проще найти событие.</p>
        ...
        <div className="mt-6">
          ...
          <label className="relative mt-3 block">
            <Search className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-white/36" size={16} />
            <UiInput
          ...
          {visibleTags.length !== displayableTags.length || visibleTags.length > shownTags.length ? (
...
function EventLocationPicker({
  ...
  useEffect(() => {
    ...
    markerNode.className = 'grid h-9 w-9 place-items-center rounded-full border border-white/40 bg-[var(--sw-accent-3)] text-white shadow-[0_12px_36px_rgba(255,123,0,0.35)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[rgba(var(--sw-accent-2-rgb),0.65)] focus-visible:ring-offset-2 focus-visible:ring-offset-black';
