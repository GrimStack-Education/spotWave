...
export function ProfileScreen() {
  ...
  const displayName =
    meData?.profile?.displayName || meData?.displayName || meData?.email || 'SpotWave User';
  const bio =
    meData?.profile?.bio ||
    meData?.bio ||
    'Заполните настройки профиля и onboarding для точной персонализации.';
  ...
  const homeLat = meData?.profile?.homeLat ?? null;
  const homeLng = meData?.profile?.homeLng ?? null;
              ...
              <div className="min-w-0">
                ...
                <p className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 text-white/58">
                  <span className="inline-flex items-center gap-2">
                    <MapPin size={16} /> Радиус {radius} км
                  </span>
                  <span className="inline-flex items-center gap-2">
                    <Mail size={16} /> {meData?.email}
                  </span>
                  <span className="inline-flex items-center gap-2">
                    <CalendarClock size={16} /> На SpotWave с {formatDate(meData?.createdAt)}
                  </span>
                </p>
              </div>
            </div>
        ...
        <UiCard className="p-6">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            <Metric
              icon={<Users size={17} />}
              label="Участий"
              value={String(activity?.joinedEventsCount ?? 0)}
            />
            <Metric
              icon={<Radio size={17} />}
              label="Хостинг"
              value={String(activity?.hostedEventsCount ?? 0)}
            />
            <Metric
              icon={<CheckCircle2 size={17} />}
              label="Check-ins"
              value={String(activity?.checkInsCount ?? 0)}
            />
            <Metric
              icon={<Star size={17} />}
              label="Рейтинг"
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {(activity?.upcomingEvents ?? []).length ? (
              activity?.upcomingEvents.map((event) => (
                ...
                >
                  <p className="text-xl leading-tight tracking-[-0.035em] text-white">
                    {event.title}
                  </p>
                  <p className="mt-3 flex items-center gap-2 text-sm text-white/52">
                    <CalendarClock size={14} /> {formatDateTime(event.startsAt)}
                  </p>
                  <p className="mt-1 flex items-center gap-2 text-sm text-white/52">
                    <MapPin size={14} /> {event.addressText ?? 'Location TBD'}
                  </p>
                </Link>
              ))
            ) : (
        ...
        <UiCard className="p-6 md:p-7">
          ...
          <div className="mt-5 space-y-3">
            ...
            <TrustRow
              ...
              value={String(trust?.openReports ?? 0)}
              tone={(trust?.openReports ?? 0) > 0 ? 'danger' : 'muted'}
            />
            <TrustRow
              label="Закрытые проверки"
              value={String(trust?.resolvedReports ?? 0)}
              tone="muted"
            />
          </div>
        </UiCard>
            ...
            {(trust?.recentReviews ?? []).length ? (
              trust?.recentReviews.map((review) => (
                ...
                >
                  ...
                  <p className="mt-3 line-clamp-3 text-sm leading-6 text-white/58">{review.text}</p>
              ...
              <p className="text-white/52">Отзывы появятся после завершенных встреч.</p>
            )}
          </div>
        </UiCard>
