...
export class CheckInsService {
  ...
  async create(eventId: string, userId: string, dto: CreateCheckInDto) {
    const event = await this.db.client.event.findUnique({ where: { id: eventId }, select: { id: true, status: true } });
    if (!event) throw new NotFoundException('Event not found');
    if (event.status !== EventStatus.ACTIVE) throw new ConflictException('Check-in available only for active events');
